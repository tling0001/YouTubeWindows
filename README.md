...
1. Install the WebView2 Runtime: https://go.microsoft.com/fwlink/p/?LinkId=2124703  
   Or place the WebView2 Shared Runtime in the runtime directory (PlayReady DRM will not be available).
...
- `Right Mouse Button` `Esc`   - Back
- `Left Mouse Button` `Enter` - Confirm
- `Arrow Keys`           - Move Cursor
- `F11`              - Fullscreen
- `F5` `Ctrl + R`    - Reload App
...
```
--allow-auto-hdr					Allows stretching SDR content to HDR (for example NVIDIA RTX HDR)
--proxy-server="http://proxy-ip:proxy-port"		Access via proxy
--enable-features={Feature1,Feature2,...}		Enable features such as Feature1, Feature2, etc. (see Microsoft Edge documentation)
```
